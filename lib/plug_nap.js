...
var MIN_WAKEUP_SEC = 30; // give enough time to shutdown properly
...
exports.newInstance = function($, spec, cb) {
    var cbOnce = myUtils.callJustOnce(function(err) {
        if (err) {
            $._.$.log && $._.$.log.debug('Ignoring >1 callback with error:' +
    ...
    try {
        ...
        d.on('error', function(err) {
            ...
            cbOnce(null, that); // continue but disable nap            
        });
	...
	d.run(function() {
            ...
            var setDS1337Time = function(t, cb0) {
                ...
                rtc.writeI2cBlock(deviceAddress, DS1337_TIME_ADDRESS, bytesBuffer.length,
                                  bytesBuffer, cb0);
            };

            var getDS1337Time = function(cb0) {
                var bytesBuffer = new Buffer(DS1337_TIME_BYTES);
                rtc.readI2cBlock(deviceAddress, DS1337_TIME_ADDRESS, DS1337_TIME_BYTES,
                                 bytesBuffer, function(err, bytesRead, res) {
                                     if (err) {
                                         cb0(err);
                                     } else if (bytesRead !== DS1337_TIME_BYTES) {
                                         var er = new Error('Not all bytes read');
                                         er.bytesRead = bytesRead;
                                         er.res = res;
                                         cb0(er);
                                         //only 24h mode supported
                                     } else if (res[2] & 0x40) {
                                         er = new Error('12h mode not  supported');
                                         er.res = res;
                                         cb0(er);
                                     } else {
                                         ...
                                         var day = BCDToDec(rtc[3]) -1;//sunday=0
            ...
            setDS1337Time(new Date(), function(err) {
                if (err) {
                    disableWithError = err;
	            $._.$.log &&
                        $._.$.log.warn('1: Disabling plug_nap due to error: ' +
                                       myUtils.errToPrettyStr(err)); 
                    cbOnce(null, that);
                } else {
		    disableWithError = null;
                    cbOnce(null, that);
                }
            });
        });
