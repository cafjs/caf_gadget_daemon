...
exports.newInstance = function($, spec, cb) {
    try {
        ...
        assert.equal(typeof(spec.env.controlVolume), 'string',
                     "'spec.env.controlVolume' is not a string");
         ...
         * and it is writing to a directory 'dir' that is in volume 
         * `internalPrefix'. 
         *
         *  We want the app container to read that directory, so we need to
         * rewrite the path of that directory, so that it is relative to 
         * `externalPrefix` instead.
         */
        var toExternalPath = function(internalPrefix, externalPrefix, dir) {
            return path.resolve(externalPrefix, path.relative(internalPrefix,
                                                              dir)); 
        };
                ...
                async.waterfall([
                    ...
                    function(tokenDir, cb2) {
                        if (tokenDir === null) {
                            ...
                            var childMailboxDir =
                                    toExternalPath('/control',
                                                   spec.env.controlVolume,
                                                   spec.env.mailboxDir);
                            var childTokenDir =
                            ...
                            that.__ca_instanceChild__(null, {
                                ...
                                env: {
                                    ...
                                    appPrivileged: spec.env.appPrivileged
                                }
                            }, cb2);
                        }
                    }
