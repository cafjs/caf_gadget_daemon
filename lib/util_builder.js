...
var imgName = function(name, etag, privileged) {
    etag = etag.replace(/[+/]/g, '0'); //docker tags cannot contain all base64
    return IMAGE_PREFIX + name + ':' + etag + (privileged ? PRIVILEGED : '');
};

var getImageId = exports.getImageId = function($, name, etag, privileged, cb0) {
    var imageName = imgName(name, etag, privileged);
    var id = null;
...
var hashMatchesEtag = function(hash, etag) {
    var splitEtag = etag.split('-');
...
exports.newImage = function($, dir, dockerfile, url, name, etag,
    ...
    async.series([
        ...
        function(cb1) {
            ...
            var extract = tar.extract({gzip: true, cwd: dir, strip: 0});
            stream.pipe(extract);
            ...
            extract.on('finish', function() {
                if (hashMatchesEtag(hashValue, etag)) {
                    ...
                    var err = new Error('Invalid hash');
                    err.hash = hashValue;
                    err.etag = etag;
                    cb2(err);
                }
        ...
        function(cb1) {
            var cb2 = callJustOnce($, cb1);
            var dirDest = fs.createWriteStream(dir + '.tgz');
            dirDest
                .on('error', cb2)
            ...
            docker.buildImage(dir + '.tgz', {
                ...
                if (err) {
                    ...
                    var onFinished = function(err, output) {
                        if (err) {
                            var error = new Error('Build error');
                            error.output = output;
                            error.err = err;
                            cb1(error);
                        } else {
