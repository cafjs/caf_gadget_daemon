    ...
    try {
        ...
        const startContainer = function(cb0) {
            ...
            if (spec.env.debugApplication) {
                ...
                createOptions.Env.push('LOG_LEVEL=DEBUG');
            }
            ...
            if (!spec.env.appPrivileged && (spec.env.appDevices.length > 0)) {
                //mount /proc/device-tree for gpio
                hostConfig.MaskedPaths = DEFAULT_MASKED_PATHS;
            }
            ...
            job.on('container', function(container) {
                $._.$.log && $._.$.log.warn('Container up, id:' +
                                            container.id);
