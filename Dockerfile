...
# TO_RUN:         docker run -d --device=/dev/i2c-1 -v /dev/initctl:/dev/initctl --name=root-rpidaemon --restart=always -e MY_ID=foo-device1 --privileged -v /var/run/docker.sock:/var/run/docker.sock -v /control:/control  -v /usr/bin/docker:/bin/docker -v /config:/config registry.cafjs.com:32000/root-rpidaemon
#
